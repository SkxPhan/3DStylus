...
class Stroke {
 ...
 public:
  ...
  Stroke(const std::vector<Point>& points, Orientation orientationSensitivity,
         int samplePointCnt = 16);
  ...
  const VectorizedStroke& getVectorizedStroke() const {
    return vectorizedStroke;
  }
};

class StrokeDetector {
  std::vector<std::pair<Stroke, std::string>> strokeTemplates;

 public:
  void addStrokeTemplate(Stroke strokeTemplate, std::string label);
  void loadStrokeTemplates(std::string path);
  ...
  std::pair<ItT, Score> recognize(const Stroke& target, ItT templateItBegin,
                                  ItT templateItEnd) {
    float maxScore = 0;
    ItT matchedIt = templateItEnd;
    for (ItT templateIt = templateItBegin; templateIt != templateItEnd;
         ++templateIt) {
      const float d = optimalCosineDistance(target.getVectorizedStroke(),
                                            templateIt->getVectorizedStroke());
  ...
  template <typename ItT>
  ItT recognize(const Stroke& target, ItT templateItBegin, ItT templateItEnd,
                Score threshold) {
