...
namespace {
float pathLength(const vector<Point>& p) {
  float length = 0;
  for (size_t i = 1; i < p.size(); ++i) {
    length +=
        std::hypotf(p[i].first - p[i - 1].first, p[i].second - p[i - 1].second);
  }
  return length;
}
...
vector<Point> resample(std::vector<Point> p, int n) {
  ...
  for (size_t i = 1; i < p.size(); ++i) {
    ...
    if (residualD + d >= len) {
      ...
      newPoints.push_back(
          std::make_pair(p[i - 1].first + frac * (p[i].first - last.first),
...
vector<Point> centerPoints(const vector<Point>& p) {
  const Point center = centroid(p);
  vector<Point> newPoints;
  newPoints.reserve(p.size());
  for (const Point& pt : p) {
    newPoints.push_back(
        std::make_pair(pt.first - center.first, pt.second - center.second));
  }
  return newPoints;
}

VectorizedStroke vectorizeStroke(const vector<Point>& p,
                                 Orientation oSensitive) {
  vector<Point> centeredPoints = centerPoints(p);
  double indicativeAngle =
      std::atan2(centeredPoints[0].second, centeredPoints[0].first);
  float delta = -indicativeAngle;
  if (oSensitive == Orientation::Sensitive) {
    static const float pi = atanf(1.0f) * 4;
    float baseOrientation =
        pi / 4.f * floorf((indicativeAngle + pi / 8.f) / (pi / 4.f));
...
float vecCrossProduct(const pair<float, float>& a,
...
Stroke::Stroke(const std::vector<Point>& points,
               Orientation orientationSensitivity, int samplePointCnt)
    : rawPoints(points),
      vectorizedStroke(vectorizeStroke(resample(points, samplePointCnt),
                                       orientationSensitivity)) {}

void StrokeDetector::addStrokeTemplate(Stroke strokeTemplate,
                                       std::string label) {
  strokeTemplates.push_back({strokeTemplate, label});
}

void StrokeDetector::loadStrokeTemplates(std::string path) {
  // Implementation TODO
}

float StrokeDetector::optimalCosineDistance(const VectorizedStroke& stroke1,
                                            const VectorizedStroke& stroke2) {
  float a = 0;
  float b = 0;
  for (size_t i = 0; i < stroke1.size(); i += 2) {
    a += vecCrossProduct(stroke1[i], stroke2[i]) +
         vecCrossProduct(stroke1[i + 1], stroke2[i + 1]);
    b += vecCrossProduct(stroke1[i], stroke2[i + 1]) +
         vecCrossProduct(stroke1[i + 1], stroke2[i]);
  }
